...
function inject (bot, { hideErrors }) {
  ...
  async function activateBlock (block, direction, cursorPos) {
    ...
    // 1.19+: server uses interaction sequences to disambiguate use_item / use_item_on.
    // If we send a constant sequence, some servers (notably Paper) can ignore the interaction,
    // causing windowOpen to never fire.
    sequence++
    // place block message
  ...
  function prepareWindow (window) {
    if (!windowItems || window.id !== windowItems.windowId) {
      ...
      if (windowItems.carriedItem !== undefined) {
        window.selectedItem = Item.fromNotch(windowItems.carriedItem)
      }
      updateHeldItem()
      extendWindow(window)
      bot.emit('windowOpen', window)
  ...
  bot._client.on('window_items', (packet) => {
    ...
    // 1.17.1+: server includes cursor item in window_items as carriedItem.
    // Newer versions (1.21+) can also send set_cursor_item; keep our local cursor state in sync.
    if (packet.carriedItem !== undefined) {
      window.selectedItem = Item.fromNotch(packet.carriedItem)
      bot.inventory.selectedItem = window.selectedItem
    }
    updateHeldItem()
    bot.emit(`setWindowItems:${window.id}`)
  })

  // 1.21+: server can send a dedicated cursor item packet.
  bot._client.on('set_cursor_item', (packet) => {
    const item = Item.fromNotch(packet.contents)
    if (bot.currentWindow) bot.currentWindow.selectedItem = item
